...
# This "low-level" library allows you to run a MPC (X) on a Force hardware
...
# Do not launch the mod if we are not running on a Force
[ "$DEVICE" != "Force" ] && exit 0
...
ASSETS_DIR=$SCRIPT_DIR/mpc-assets

# Splash screen  
SPLASH_SCREEN=$ASSETS_DIR/mpc_splash270_tkgl.data
...
MPCMAPPER_CONFIG_FILE=$CHROOT_BOOTSTRAP/modules/mod_iamx/conf/map_x.conf

# mpcmapper command line arguments
TKGL_ARGV="--tkgl_iamX --tkgl_configfile=$MPCMAPPER_CONFIG_FILE"
...
ROOTFS_IMG_NAME="$ASSETS_DIR/rootfs_mpc-2.10.1.83.img"
...
echo "Mounting point of the MPC OS img        : $ROOT_DIR"  >> $TKGL_LOG
...
# start to chroot in the MPC image -------------------------------------------
...
echo "Welcome to The Kikgen Labs world !\nIamX preliminary version">$ROOT_DIR$MPC_MESSAGEINFO
